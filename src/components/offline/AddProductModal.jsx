...
export default function AddProductModal({ onClose, onAddProducts }) {
  ...
  // OCR Flow states
  const [ocrTarget, setOcrTarget] = useState(null); // { file, url }
  ...
  const handleParsedProducts = (products, text) => {
    setScannedItems(products);
    setOcrTarget(null); // Reset OCR state
  };

  const handleConfirmScan = (items) => {
    onAddProducts(items);
    onClose();
  };
            ...
            <>
              ...
              <div className="min-h-[300px]">
                ...
                {activeTab === 'ocr' && (
                  ocrTarget ? (
                    <OCRScanner 
                      imageFile={ocrTarget.file} 
                      imageUrl={ocrTarget.url} 
                      onScanComplete={handleParsedProducts} 
                    />
                  ) : (
                    <div className="max-w-xl mx-auto">
                      <ProductImageUploader onImageSelect={setOcrTarget} />
                    </div>
                  )
                )}
